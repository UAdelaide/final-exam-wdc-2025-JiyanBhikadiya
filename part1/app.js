...
app.get('/api/walkers/summary',async (req,res,next) => {
    try{
        const [summary] = await db.execute(`
            SELECT Users.username AS walker_username,COUNT(WalkRatings.rating_id) AS total_ratings,AVG(WalkRatings.average_rating) AS average_rating,(SELECT * FROM WalkRequests JOIN WalkApplications ON WalkApplications.request_id )
        `);
        res.json(summary);
    }catch(error){
